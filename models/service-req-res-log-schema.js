...
module.exports.getRecords = async (data) => {
  var query = {};
  const { company_id, service_id, from_date, to_date, type, page } = data;
  if (company_id) {
    query['$and'] = [];
    query['$and'].push({
      company_id,
  ...
  const serviceResp = await ServiceReqResLog.find(query)
    .skip(page * 15)
    .limit(15);
  const count = await ServiceReqResLog.find(query).count();
...
module.exports.insertCodeAndTime = (data) => {
  return ServiceReqResLog.updateMany(
...
module.exports.findByIdAndAPIName = async (request_id, api_name) => {
  try {
      return await ServiceReqResLog.findOne({
...
module.exports.findByIdAndReqandResTypeAndAPIName = async (request_id, api_name, req_type, resp_type) => {
  try {
      return await ServiceReqResLog.findOne({
          request_id: request_id,
          api_name: api_name,
