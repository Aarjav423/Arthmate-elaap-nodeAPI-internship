...
const leadMappingSchema = require('../models/lead-mapping-schema.js');
const loanMappingSchema = require('../models/disbursement-status-schema.js');
...
const appendLoanIdBwId = (data, loanReqData, req, res) => {
  data.forEach(async (item) => {
    let aadhaarId = item.aadhar_card_num;
    ...
    if (item.hasOwnProperty('appl_pan') && item.appl_pan) {
      item['borrower_id'] = `${item.appl_pan.substring(0, 5)}${crypto
        .randomBytes(2)
        .toString('hex')}${item.appl_pan.substring(9, 10)}`;
    } else if (item.hasOwnProperty('aadhar_card_num') && item.aadhar_card_num) {
      item['borrower_id'] = `${item.aadhar_card_num.substring(0, 5)}${crypto
        .randomBytes(2)
    ...
    const colenders = req.product.co_lenders;
    var isDirectAssignment = '';
    var colendingMode = '';
    var _colendingShortCode = '';
    for await (let ele of colenders) {
      colendingMode = (
        await ColenderProfileSchema.findByColenderId(ele.co_lender_id)
      )?.co_lending_mode;
      _colendingShortCode = (
        await ColenderProfileSchema.findByColenderId(ele.co_lender_id)
      )?.co_lender_shortcode;
      if (process.env.CO_LENDER_KYC_SPLIT.includes(_colendingShortCode)) {
        if (colendingMode === 'DA' || colendingMode === 'RT') {
    ...
    if (isDirectAssignment === true) {
      // Save AES-256 encrypted aadhaar number
      var algorithm = 'aes-256-cbc';
      const key = Buffer.from(process.env.CBI_PUBLIC_KEY, 'base64');
      const iv = Buffer.from(process.env.CBI_PUBLIC_IV, 'base64');
      if (aadhaarId) {
        const encodedAadhaarStart = encrypt(aadhaarId.substring(0, 4), key, iv);
        let leadMappingObject = {
          borrower_no_hash: encodedAadhaarStart,
          loan_app_id: item.loan_app_id,
          product_id: req.product._id,
        };
        await leadMappingSchema.addNew(leadMappingObject);
        const encodedAadhaarMid = encrypt(aadhaarId.substring(4, 8), key, iv);

        let loanMappingObject = {
          disbursement_seq: encodedAadhaarMid,
          loan_app_id: item.loan_app_id,
          product_id: req.product._id,
        };
        await loanMappingSchema.addNew(loanMappingObject);
      }
    }
  });
  return data;
...
const encrypt = (plainString, AesKey, AesIV) => {
...
const ckycSearchMatchPattern = (type, value) => {
  switch (type) {
    case 'E':
      const UID =
        /^[0-9]{4}[|][a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*[|]\d{2}-\d{2}-\d{4}[|][A-Z]{1}$/;
...
const addUpdateDuesOnLoan = (record, dues, updateUsage) => {
  ...
  dues.forEach((due) => {
    ...
    var temp_exclude_interest_till_grace_period =
      record.exclude_interest_till_grace_period
    ...
    record.exclude_interest_till_grace_period =
      temp_exclude_interest_till_grace_period.toString();
...
const addUpdateDuesOnLoanJoined = (records, updateUsage) => {
  ...
  records.forEach((due) => {
    if (record.d_txn_id) {
      ...
      var temp_exclude_interest_till_grace_period =
        record.exclude_interest_till_grace_period.toString()
          ? record.exclude_interest_till_grace_period.toString().split()
          : [];
      temp_exclude_interest_till_grace_period.push(
  ...
  CLTransactionSchema.getClTransactionCount(
    ...
    (errorCountUsage, respCountUsage) => {
      ...
      currentUsageBIConfig['interest_free_days'] =
        ...
        0;
      currentUsageBIConfig['exclude_interest_till_grace_period'] =
