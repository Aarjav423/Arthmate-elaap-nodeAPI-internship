    ...
    try {
      if (req.authData.type === 'service' || req.authData.type === 'dash') {
        ...
        if (service.status !== 1)
          throw {
            message:
              'This service is currently inactive. Kindly contact support team.',
          };
        req.service = service;
        return next();
      } else {
  ...
  return compose().use(async (req, res, next) => {
    try {
      if (req.authData.type === 'service' || req.authData.type === 'dash') {
        ...
        const companyServicesRes = await CompanyServices.findOneWithCompanyId(
          req.company._id,
        );
        if (!companyServicesRes)
          return res.status(400).json({
            message:
              'This service is not available for your company. Kindly contact support team.',
          });
...
const isPaymentServiceEnabled = (sid) => {
  return compose().use((req, res, next) => {
    Services.findOneWithId(sid, (servicesErr, servicesRes) => {
      ...
      req.service = servicesRes;
      //Find company service with company id
      CompanyServices.findOneWithCompanyId(
        req.company._id,
        (companyServiceErr, companyServicesRes) => {
...
const getCibilV3PostData = (config) => {
  var { userID, password_userID, data, memberCode, memberCode_password } =
    config;
