...
const maintainAccessLog = async (req, res, next) => {
  try {
    ...
    data.company_code =
      (authData && authData.type === 'api') ||
      (authData && authData.type === 'service') ||
      (authData && authData.type === 'dash-api')
        ? authData.company_code
        : authData && authData.type === 'dash'
        ? 'Null'
        : 'Null';
...
const queueMaintainAccessLog = (req, res, next) => {
  ...
  const accessLogpath = `pan-service-api-accesslog/${
    data.company_code
  }/${moment().format('YYYY-MM-DD')}/${uuid.v4()}.json`;
...
const billingAccesslog = (data, callback) => {
  const billingLogpath = `pan-service-api-billinglog/${
    data.company_code
  }/${moment().format('YYYY-MM-DD')}/${uuid.v4()}.json`;
...
const ekyProcessStoreAccessLog = (data, panData, callback) => {
  ...
  const ekyProcessStoreAceessLogPath = `pan-ekyprocess-store/${
    panData.company_code
  }/${moment().format('YYYY-MM-DD')}/${uuid.v4()}.json`;
  helper.uploadFileToS3(
    data,
    ekyProcessStoreAceessLogPath,
