...
const verifyPanKyc = (pan_data_url, lrData) => {
  return new Promise(async (resolve, reject) => {
    const resultbiJson = await s3helper
      .asyncFetchJsonFromS3(pan_data_url, {
        method: 'Get',
      })
      .catch((err) => {
        return err;
...
const verifyCkyc = async (ckyc_url, lrData) => {
  try {
    ...
    if (ckycData.DOB) {
      if (moment(lrData.dob).format('DD-MM-YYYY') != ckycData.DOB[0])
        throw {
          success: false,
      ...
      var age =
        moment().format('YYYY') -
        parseInt(moment(lrData.dob).format('YYYY'), 10);
      if (age != ckycData.AGE[0])
        throw {
          success: false,
    ...
    if (ckycData.PAN) {
      if (lrData.appl_pan != ckycData.PAN[0])
        throw {
          success: false,
...
const verifyEkyc = async (ekyc_url, lrData) => {
  try {
    ...
    if (
      lrData.pincode != resultbiJson.pincode &&
      lrData.per_pincode != resultbiJson.pincode
    )
      throw {
        success: false,
        message: 'Pincode is incorrect.',
      };
    if (lrData.dob != resultbiJson.dob)
      throw {
        success: false,
  ...
  try {
    ...
    if (lrData.borro_bank_acc_num !== resultbiJson.data.result.accountNumber)
      throw {
        success: false,
        message:
          'The bank account number associated with your loan is not verified',
      };
...
const CheckMandatoryDocUpload = async (loan_schema_id, loan_id) => {
  ...
  try {
    ...
    const loanTempRes = await LoanTemplateSchema.findByNameTmplId(
      loanschemaRes.loan_custom_templates_id,
      'loandocument',
    );
    ...
    for (var i = 0; i < mandatoryDocArray.length; i++) {
      for (var j = 0; j < loanDocCommonfindres.length; j++) {
        if (
          !loanDocumentsData.hasOwnProperty(mandatoryDocArray[i]) &&
          loanDocCommonfindres[j][mandatoryDocArray[i]]
        )
          loanDocumentsData[mandatoryDocArray[i]] =
            loanDocCommonfindres[j][mandatoryDocArray[i]];
      }
    }
