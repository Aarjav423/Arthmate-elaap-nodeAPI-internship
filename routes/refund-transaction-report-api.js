...
module.exports = (app, connection) => {
  ...
  //Api to fetch generated refund records
  app.get(
    '/api/refund-transaction-reports',
    [jwt.verifyToken],
    async (req, res) => {
      try {
        const company_id = req.authData.company_id;
        const refundTransactionReportsResp =
        ...
        if (!refundTransactionReportsResp.rows.length)
          throw {
            success: false,
            message: ' No records found for refund transaction reports',
          };
        return res.status(200).send(refundTransactionReportsResp);
      } catch (error) {
  ...
  //Api to download generated refund transaction  report
  app.get(
    '/api/download-refund-transaction-report/:id',
    [jwt.verifyToken],
    async (req, res) => {
      try {
        const refundTransactionReportResp = await ReportStorageSchema.findById(
          req.params.id,
        );
        if (!refundTransactionReportResp)
          throw {
            success: false,
            message: 'No record found for refund transaction report.',
          };
        const url = refundTransactionReportResp.s3_url.substring(
          refundTransactionReportResp.s3_url.indexOf('refund-transaction'),
        );

        const reportFromS3Resp = await s3helper.readFileFromS3(url);
        res.attachment(url);
        let filename = `refundTransactionReport${Date.now()}.xlsx`;
  ...
  // Api to fetch filtered refund transaction records
  app.post(
    '/api/filtered_refund_transaction_records',
    [
    ...
    async (req, res) => {
      try {
        ...
        const refundTransactionRecords =
          await DisbursementAndTopupSchema.getFilteredRefundRecords(data);
        if (!refundTransactionRecords.length)
          throw {
            success: false,
            message: 'No refund records found against provided filter',
          };
        return res.status(200).send(refundTransactionRecords);
      } catch (error) {
        return res.status(400).send(error);
      }
    },
  ...
  app.post(
    ...
    [jwt.verifyToken, jwt.verifyUser, jwt.verifyCompanyForAllPartners],
    async (req, res) => {
      try {
        const data = req.body;
        const errors = validationResult(req);
        ...
        // Fetch refunds records according to the filters
        const refundRecords =
          await DisbursementAndTopupSchema.getFilteredRefundRecords(data);
        ...
        Array.from(refundRecords).forEach((refundRecords) => {
          Array.from(borrowerInfos).forEach((borrowerInfo) => {
            if (refundRecords.loan_id === borrowerInfo.loan_id) {
              reportPayload.push({
                'Partner borrower id': refundRecords.partner_borrower_id || '',
                'Borrower id': refundRecords.borrower_id || '',
                'Partner loan id': refundRecords.partner_loan_id || '',
                'Loan id': refundRecords.loan_id || '',
                'Disbursement status': refundRecords.webhook_status || '',
                'UTRN number': refundRecords.utrn_number || '',
                'Txn date': refundRecords.disbursement_date_time || '',
                'Transaction amount': refundRecords.amount || '',
                'Customer name':
                ...
                Remarks: refundRecords.bank_remark || '',
