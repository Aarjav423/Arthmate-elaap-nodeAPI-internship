...
module.exports = (app, connection) => {
  ...
  app.post(
    ...
    [
      ...
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_CRIF_ID),
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        const ReferenceNo =
          'b0Yf0000003fL7xEA456' +
          Math.floor(1000000000000 + Math.random() * 9000000000000);
        const postData = `<REQUEST-REQUEST-FILE><HEADER-SEGMENT><SUB-MBR-ID>MAMTA PROJECTS PRIVATE LIMITED</SUB-MBR-ID><INQ-DT-TM>${moment().format(
          'DD-MM-YYYY HH:mm:ss',
        )}</INQ-DT-TM><REQ-ACTN-TYP>SUBMIT</REQ-ACTN-TYP><TEST-FLG>HMTEST</TEST-FLG><AUTH-FLG>Y</AUTH-FLG><AUTH-TITLE>USER</AUTH-TITLE><RES-FRMT>XML</RES-FRMT><MEMBER-PRE-OVERRIDE>N</MEMBER-PRE-OVERRIDE><RES-FRMT-EMBD>Y</RES-FRMT-EMBD><MFI><INDV>true</INDV><SCORE>false</SCORE><GROUP>true</GROUP></MFI><CONSUMER><INDV>true</INDV><SCORE>true</SCORE></CONSUMER><IOI>true</IOI></HEADER-SEGMENT><INQUIRY><APPLICANT-SEGMENT><APPLICANT-NAME><NAME1>${
          data.borrower_name_1
        }</NAME1><NAME2>${
          data.borrower_name_2
        }</NAME2><NAME3></NAME3><NAME4></NAME4><NAME5></NAME5></APPLICANT-NAME><EMAILS><EMAIL>${
          data.email_id
        }</EMAIL></EMAILS><DOB><DOB-DATE>${moment(data.dob).format(
          'DD/MM/YYYY',
        )}</DOB-DATE><AGE>${
          data.borrower_age
        }</AGE><AGE-AS-ON>${moment().format(
          'DD-MM-YYYY',
        )}</AGE-AS-ON></DOB><IDS><ID><TYPE>${
          data.borrower_id_type
        }</TYPE><VALUE>${data.borrower_id_number}</VALUE></ID></IDS><GENDER>${
          data.gender
        }</GENDER><RELATIONS><RELATION><NAME>${
          data.borrower_father_name
        }</NAME><TYPE>K01</TYPE></RELATION></RELATIONS><KEY-PERSON><NAME>${
          data.borrower_mother_name
        }</NAME><TYPE>K03</TYPE></KEY-PERSON><NOMINEE><NAME>${
          data.borrower_nominee_name
        }</NAME><TYPE>K01</TYPE></NOMINEE><PHONES><PHONE><TELE-NO>${
          data.borrower_telephone_num
        }</TELE-NO><TELE-NO-TYPE>${
          data.borrower_telephone_num_type
        }</TELE-NO-TYPE></PHONE></PHONES></APPLICANT-SEGMENT><ADDRESS-SEGMENT><ADDRESS><TYPE>${
          data.borrower_address_type
        }</TYPE><ADDRESS-1>${data.borrower_address}</ADDRESS-1><CITY>${
          data.borrower_city
        }</CITY><STATE>${data.borrower_state}</STATE><PIN>${
          data.borrower_pincode
        }</PIN></ADDRESS></ADDRESS-SEGMENT><APPLICATION-SEGMENT><INQUIRY-UNIQUE-REF-NO>${
          moment().format('DDMMYYYYHHmmss') + ReferenceNo
        }</INQUIRY-UNIQUE-REF-NO><CREDT-INQ-PURPS-TYP>${
          data.enquiry_purpose
        }</CREDT-INQ-PURPS-TYP><CREDIT-INQUIRY-STAGE>${
          data.enquiry_stage
        }</CREDIT-INQUIRY-STAGE><CREDT-REQ-TYP>INDV</CREDT-REQ-TYP><BRANCH-ID>3008</BRANCH-ID><LOS-APP-ID>${ReferenceNo}</LOS-APP-ID><LOAN-AMOUNT>${
          data.loan_amount
        }</LOAN-AMOUNT></APPLICATION-SEGMENT></INQUIRY></REQUEST-REQUEST-FILE>`;
        ...
        var cachedBureau = await bureau_data.findIfExists(
          req.body.loan_app_id,
          req.body.borrower_id_number,
          'SUCCESS',
          'CRIF',
        );
        ...
        axios(config)
          .then(async (response) => {
            ...
            if (req.body.consent === 'Y') {
              const crif_res_data = await addBureauData(
                req.body,
          ...
          .catch(async (error) => {
            ...
            if (req.body.consent === 'Y') {
              const crif_data = await addBureauData(
                req.body,
                uploadResponse.Location,
                uploadXmlDataResponse.Location,
                req.company._id,
                req.company.code,
                'FAIL',
                requestID,
  ...
  app.post(
    ...
    [
      ...
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_CRIF_MINIMAL_ID),
      AccessLog.maintainAccessLog,
      verifyloanAppIdValidation,
    ],
    async (req, res, next) => {
      ...
      try {
        ...
        const postData = `<REQUEST-REQUEST-FILE><HEADER-SEGMENT><SUB-MBR-ID>MAMTA PROJECTS PRIVATE LIMITED</SUB-MBR-ID><INQ-DT-TM>${moment().format(
          ...
          data.name
        }</NAME1><NAME2></NAME2><NAME3></NAME3><NAME4></NAME4><NAME5></NAME5></APPLICANT-NAME><DOB><DOB-DATE>${moment(
          data.dob,
        ).format(
          'DD/MM/YYYY',
        )}</DOB-DATE><AGE></AGE><AGE-AS-ON></AGE-AS-ON></DOB><IDS><ID><TYPE></TYPE><VALUE></VALUE></ID></IDS><RELATIONS><RELATION><NAME>${
          data.father_name
        }</NAME><TYPE>K01</TYPE></RELATION></RELATIONS><KEY-PERSON><NAME></NAME><TYPE></TYPE></KEY-PERSON><NOMINEE><NAME></NAME><TYPE></TYPE></NOMINEE><PHONES><PHONE><TELE-NO>${
          data.phone_number
        }</TELE-NO><TELE-NO-TYPE>P01</TELE-NO-TYPE></PHONE></PHONES></APPLICANT-SEGMENT><ADDRESS-SEGMENT><ADDRESS><TYPE>D01</TYPE><ADDRESS-1>${
          data.address
        }</ADDRESS-1><CITY>${data.city}</CITY><STATE>${
          data.state
        }</STATE><PIN>${
          data.pincode
        }</PIN></ADDRESS></ADDRESS-SEGMENT><APPLICATION-SEGMENT><INQUIRY-UNIQUE-REF-NO>${
          moment().format('DDMMYYYYHHmmss') + ReferenceNo
        }</INQUIRY-UNIQUE-REF-NO><CREDT-INQ-PURPS-TYP>ACCT-ORIG</CREDT-INQ-PURPS-TYP><CREDIT-INQUIRY-STAGE>PRE-DISB</CREDIT-INQUIRY-STAGE><CREDT-REQ-TYP>INDV</CREDT-REQ-TYP><BRANCH-ID>3008</BRANCH-ID><LOS-APP-ID>${ReferenceNo}</LOS-APP-ID><LOAN-AMOUNT></LOAN-AMOUNT></APPLICATION-SEGMENT></INQUIRY></REQUEST-REQUEST-FILE>`;
        ...
        const reqKey = `${
          objData.api_name
        }/${vendor_name}/${company_id}/${filename}/${Date.now()}.txt`;
        //upload request data on s3
        const uploadResponse = await helper.uploadFileToS3(req.body, reqKey);
        if (!uploadResponse) {
          (objData.document_uploaded_s3 = 0), (objData.response_type = 'error');
