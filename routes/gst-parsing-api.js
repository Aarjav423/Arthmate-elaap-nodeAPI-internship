...
async function createLocalLog(s3LogResponse, localLogData, isRequest) {
  ...
  if (!insertResult)
    throw { message: 'Error while adding service log data', success: false };
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const files = new Buffer.from(req.body.file, 'base64');
        let originalname = 'GST_PDF_PARSING.pdf';

        const outForm = new FormData();
        outForm.append('file', files, originalname);
  ...
  app.post(
    ...
    [
      ...
      services.isServiceEnabled(
        process.env.SERVICE_GST_PDF_PARSING_DOWNLOAD_ID,
      ),
      verifyloanAppIdValidation,
    ],
