    ...
    async (req, res) => {
      ...
      try {
        ...
        if (cachedBureau[0]) {
          ...
          const serviceLogging = await ServiceReqResLog.addNew(localLogData);
        ...
        if (apiResponse?.data?.controlData?.success === true) {
        ...
        } else if (apiResponse?.data?.controlData?.success === false) {
          localLogData.response_type = 'error';
          localLogData.api_response_status = 'FAIL';
          const errorUrls = await createLog(
            apiResponse?.data,
            localLogData,
            false,
          );
        ...
        if (error.message.validationmsg) {
          return res.status(400).send({
            request_id: requestID,
