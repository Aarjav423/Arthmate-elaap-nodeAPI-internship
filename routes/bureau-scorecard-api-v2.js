...
async function createLocalLog(s3LogResponse, localLogData, isRequest) {
  ...
  if (s3LogResponse) {
    ...
    localLogData.response_type = localLogData.response_type
      ? localLogData.response_type
      : 'success';
    localLogData.api_response_status = localLogData.api_response_status
      ? localLogData.api_response_status
      : 'SUCCESS';
...
async function makeCrifCall(requestBody, gen_request_id) {
  try {
    var reqData = {
      ...
      gender:
        requestBody.gender.toUpperCase() === 'MALE'
          ? 'G01'
          : requestBody.gender.toUpperCase() === 'FEMALE'
          ? 'G02'
          : 'G03',
...
async function makeCibilCall(requestBody, gen_request_id) {
  try {
    ...
    const address = generateAddress(requestBody.address);
    let reqData = {
      ...
      name_birth_date_1:
        moment(requestBody.dob).format('DD') +
        moment(requestBody.dob).format('MM') +
        moment(requestBody.dob).format('YYYY'),
      name_gender_1:
        requestBody.gender.toUpperCase() === 'FEMALE'
          ? '1'
          : requestBody.gender.toUpperCase() === 'MALE'
...
async function makeExperianCall(requestBody, gen_request_id) {
  const reqData = {
    ...
    number_of_major_credit_card_held:
      requestBody.number_of_major_credit_card_held,
...
module.exports = (app) => {
  ...
  app.post(
    ...
    async (req, res) => {
      const gen_request_id = `${
        req.company.code
      }-BUREAU-SCORECARD-V2-${Date.now()}`;
      try {
        ...
        localLogData.request_id = gen_request_id;
        ...
        const leadData = await loanRequestSchema.findByLId(
          req.body.loan_app_id,
        );
        if (!leadData) {
          throw {
            message: 'loan_app_id not found, please provide valid loan_app_id',
            success: false,
          };
        }
        ...
        await BureauScorecards.updateBureauRequest(
          gen_request_id,
          createdLogs.raw_data,
        );
        // invoke one-of-the-bureau apis (CRIF/CIBIL/EXPERIAN)
        const bureauResponse = await makeBureauCall(
          req.body.bureau_type,
          req.body,
          gen_request_id,
        );
        if (
          req.body?.bureau_type === 'cibil' &&
          bureauResponse?.data?.result?.controlData?.success !== true
        ) {
          localLogData.response_type = 'error';
          localLogData.api_response_status = 'FAIL';
        }
        ...
        await BureauScorecards.updateBureauResponse(
          gen_request_id,
        ...
        if (bureauResponse.data) {
          ...
          var resData =
            req.body.bureau_type == 'cibil'
              ? bureauResponse.data.result
              : bureauResponse.data.data;
          if (!resData) {
            throw {
              message: 'Unable to fetch bureau response using this data',
              success: false,
          ...
          CommonBureauMapper_v2(
            req,
            req.body.loan_app_id,
            req.body.bureau_type,
            req.company._id,
            resData,
            req.body.enquiry_amount,
            req.body.tenure,
            req.body.product_type,
            gen_request_id,
            (err, bureauMapperRes) => {
              if (err) {
                BureauScorecards.updateMappedResponse(
                  gen_request_id,
                  err,
                  'FAILED',
                );
        ...
        await createLog(error, localLogData, false);
        return res.status(400).send({
          data: null,
          success: false,
          message: error
  ...
  app.post('/api/bureau-scorecard-webhook', async (req, res) => {
    try {
      if (req.headers.authorization === process.env.BUREAU_WEBHOOK_TOKEN) {
        const createSchemaResult = await BureauScoreWebhookSchema.add({
          ...req.body,
        });
        res.status(200).send({ message: 'Webhook Data Received' });

        fetchedScoreDetails = await BureauScorecards.findByGenRequestId(
          req.body.request_id,
        );
        const notificationDetails = {
          company_id: fetchedScoreDetails.company_id,
          product_id: fetchedScoreDetails.product_id,
          requestBody: req.body,
          eventKey: 'a_score',
          aScoreRequestId: req.body.request_id,
        };

        //upload request data to s3 and store it in webhook_notify_calls table
        let filename =
          Math.floor(10000 + Math.random() * 99999) + '_ascore_req';
        const reqKey = `WEBHOOK-NOTIFY/${notificationDetails.company_id}/${
          notificationDetails.product_id
        }/${notificationDetails.aScoreRequestId}/${filename}/${Date.now()}.txt`;
        ...
        // Webhook Notification db logging
        await WebhookNotifySchema.recordAScoreRequestData({
          company_id: fetchedScoreDetails?.company_id,
          product_id: fetchedScoreDetails?.product_id,
          transaction_id: req.body.request_id,
        ...
        const requestKey = `CALLBACK-REQUEST-BODY/${
          notificationDetails.company_id
        }/${notificationDetails.product_id}/${
          notificationDetails.aScoreRequestId
        }/${fname}/${Date.now()}.txt`;
        ...
        await PartnerNotificationSchema.recordAScoreRequestData({
          loan_app_id: fetchedScoreDetails?.loan_app_id,
          request_id: req.body?.request_id,
          company_id: fetchedScoreDetails?.company_id,
          product_id: fetchedScoreDetails?.product_id,
          request_s3_url: uploadWebhookRequestToS3?.Location,
          stage: 0,
          remarks: 'a-score request id received from risk team',
          key: 'a_score',
        });
      } else {
        throw {
          message: 'Invalid Token or Score Not Found',
          success: false,
        };
      }
    } catch (error) {
