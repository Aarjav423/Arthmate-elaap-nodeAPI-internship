...
module.exports = (app, connection) => {
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        const dataToHash = req.body.aadhaar_no;
        const hashAlgorithm = 'sha256'; // You can choose other algorithms like 'md5', 'sha1', etc.

        const hashedAadhaar = crypto.createHash(hashAlgorithm).update(dataToHash).digest('hex');
        //Karza data
        const karzaData = {
        ...
        //Update aadhaar details in loanrequests schema
        await LeadSchema.updateByLAid({
  ...
  // Aadhaar-xml-file-download Version 2 API
  app.post(
    '/api/kz-aadhaar-xml-file-v2',
    [
      jwt.verifyToken,
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabledCached(
        process.env.SERVICE_KZ_AADHAAR_XML_FILE_V2_ID,
      ),
      AccessLog.maintainAccessLog,
      verifyloanAppIdValidation,
    ],
    async (req, res, next) => {
      try {
        ...
        //Karza data with aadhaar number addition
        const karzaData = {
          requestId: req.body.request_id,
          otp: req.body.otp,
          consent: req.body.consent,
        ...
        //declrations for data saving
        const apiName = 'AADHAAR-XML-FILE-V2';
        const serviceid = process.env.SERVICE_KZ_AADHAAR_XML_FILE_V2_ID;
  ...
  axios
    ...
    .then(async (response) => {
      ...
      if (logData.api_response_status == 'SUCCESS') {
        ...
        if(logData.api_name !== "AADHAAR-XML-OTP"){
        //Update aadhaar details in loanrequests schema
        await LeadSchema.updateAadhaarDetails({
          aadhar_verified:"Y"
          },req.body.loan_app_id);
        }
