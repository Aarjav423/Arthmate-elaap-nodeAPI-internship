...
module.exports = (app, connection) => {
  ...
  app.post(
    ...
    [
      ...
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_CRIF_SOFT_PULL_ID),
      AccessLog.maintainAccessLog,
      verifyloanAppIdValidation,
    ],
    async (req, res) => {
      const apiName = 'CRIF-SOFT-PULL';
      ...
      try {
        ...
        const accesscode = `${process.env.CRIF_SOFT_PULL_USERNAME}|${
          process.env.CRIF_SOFT_PULL_MERCHANT_ID
        }|${process.env.CRIF_SOFT_PULL_PRODUCT_ID}|${
          process.env.CRIF_SOFT_PULL_PASSWORD
        ...
        const objData = {
          ...
          service_id: process.env.SERVICE_CRIF_SOFT_PULL_ID
            ? process.env.SERVICE_CRIF_SOFT_PULL_ID
            : '0',
        ...
        var cachedBureau = await bureau_data.findCachedBureauGeneric(
        ...
        var crifDataDetails = await CrifData.findLoanIdData(
          req.body.loan_app_id,
        );
        if (!crifDataDetails[0]) {
          ...
          const bureaurLogSchemaResponse1 = await helper.uploadFileToS3(
            apiResponse.data,
            resKey1,
          );
          objData.request_type = 'response';
        ...
        if (crifDataDetails[0]) {
          ...
          //upload request data on s3
          const bureaurLogSchemaResponse2 = await helper.uploadFileToS3(
            apiResponse2.data,
            resKey2,
          );
          objData.request_type = 'response';
          ...
          //upload request data on s3
          const bureaurLogSchemaResponse3 = await helper.uploadFileToS3(
            apiResponse3.data,
            resKey3,
          );
          objData.request_type = 'response';
