...
const RepaymentScheduleSchema = require('../models/repayment-installment-schema');
...
module.exports = (app, connection) => {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const repaymentScheduleReportsResp =
        ...
        if (!repaymentScheduleReportsResp.rows.length)
          throw {
            success: false,
            message: ' No records found for repayment schedule reports',
          };
        return res.status(200).send(repaymentScheduleReportsResp);
      } catch (error) {
  ...
  //Api to download generated repayment schedule report
  app.get(
    '/api/download-repayment-schedule-report/:id',
    [jwt.verifyToken],
    async (req, res) => {
      try {
        const repaymentScheduleReportResp = await ReportStorageSchema.findById(
          req.params.id,
        );
        if (!repaymentScheduleReportResp)
          throw {
            success: false,
            message: 'No record found for repayment schedule report.',
          };
        const url = repaymentScheduleReportResp.s3_url.substring(
          repaymentScheduleReportResp.s3_url.indexOf('repayment_schedule'),
        );
        const reportFromS3Resp = await s3helper.readFileFromS3(url);
        res.attachment(url);
        let filename = `repaymentScheduleReport${Date.now()}.xlsx`;
    ...
    async (req, res) => {
      try {
        ...
        if (!borrowersInfo)
          throw {
            success: false,
            message: 'No records found against provided filter 123',
          };

        data.loanIds = borrowersInfo.map((record) => {
          return record.loan_id;
        });
        const repaymentScheduleRecords =
          await RepaymentScheduleSchema.getRecordsByLidCidAndPid(data);
        if (!repaymentScheduleRecords.length)
          throw {
            success: false,
            message:
              'No repayment schedule records found against provided filter',
          };

        const reportPayload = [];
        Array.from(borrowersInfo).forEach((borrowerInfo) => {
          Array.from(repaymentScheduleRecords).forEach(
            (repaymentScheduleRecord) => {
              if (borrowerInfo.loan_id === repaymentScheduleRecord.loan_id) {
                reportPayload.push({
                  'Partner loan id': borrowerInfo.partner_loan_id || '',
                  'Loan id': repaymentScheduleRecord.loan_id || '',
                  'Opening principal':
                    repaymentScheduleRecord.principal_outstanding || '',
                  'Installment number': repaymentScheduleRecord.emi_no || '',
                  'Due date':
                    moment(repaymentScheduleRecord.due_date).format(
                      'YYYY-MM-DD',
                    ) || '',
                  'Due amount': repaymentScheduleRecord.emi_amount || '',
                  Principal: repaymentScheduleRecord.prin || '',
                  Interest: repaymentScheduleRecord.int_amount || '',
                  'Closing principal':
                    repaymentScheduleRecord.principal_bal || '',
                  'Disbursement date':
                    borrowerInfo.disbursement_date_time || '',
                });
              }
            },
          );
        ...
        let fileName = `RepaymentSchedule_${req?.company?.code}_${
        ...
        const filePathInS3 = `repayment_schedule/${req?.company?.code}/${
          data.product_id ? data.product_id : 'all'
        ...
        let reportData = {
          ...
          report_name: 'repayment_schedule',
          from_date: data.from_date,
          to_date: data.to_date,
        };
        ...
        if (!recordGenereatedReport)
          throw {
            success: false,
            message: 'Error while recording generated schedule report',
          };
        return res.status(200).send({
          message: 'Repayment schedule report generated successfully.',
