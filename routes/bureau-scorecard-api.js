...
const TAG = 'bureau-scorecard-api';
const getAge = (dob) => {
...
module.exports = (app) => {
  ...
  app.post(
    ...
    [
      ...
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_BUREAU_SCORECARD_ID),
    ],
    async (req, res) => {
      ...
      try {
        ...
        const localLogData = initLocalLogData(req, {
          service_id: process.env.SERVICE_BUREAU_SCORECARD_VERIFY_ID,
          api_name: 'BUREAU-SCORECARD',
        ...
        if (bureauResponse && bureauResponse.data) {
          ...
          const responseUrl = await createLog(resData, localLogData, false);
  ...
  app.post(
    '/api/bureau-scorecard-hook',
    [
      jwt.verifyToken,
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_BUREAU_SCORECARD_HOOK_ID),
    ],
    async (req, res) => {
      try {
        // validate hook-data with service template
        // read client info from databse
        const clientInfo = (
          await BureauScorecards.findByGenRequestId(req.body.requestId)
        ).client_data;

        // initialize local-logging object
        const localLogData = initLocalLogData(req, {
          service_id: process.env.SERVICE_BUREAU_SCORECARD_HOOK_ID,
          api_name: 'BUREAU-SCORECARD-HOOK',
          pan_card: clientInfo.pan,
        });

        // log received hook-data
        await createLog(req.body, localLogData, true);

        // read scorecard result json from hookData
        const scorecardResult = req.body;

        if (scorecardResult) {
          // acknowledge provider of the received data
          res.status(200).send({
            data: null,
          ...
          // save flask response
          const updateFlaskResponseResult =
            await BureauScorecards.updateFlaskResponse(
              req.body.requestId,
              scorecardResult,
            );
          ...
          return res
            .status(400)
            .send({ data: null, message: 'Something went wrong!' });
        }
      } catch (error) {
        return res
          .status(400)
  ...
  // get all bureau-scorecard
  app.get(
    '/api/bureau-scorecard-reports',
    [
      jwt.verifyToken,
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(
        process.env.SERVICE_BUREAU_SCORECARD_REPORTS_ID,
      ),
    ],
    async (req, res) => {
      try {
        ...
        const result = await BureauScorecards.getAll(offset, limit);
  ...
  // get bureau-scorecard
  app.get(
    '/api/bureau-scorecard-report',
    [
      jwt.verifyToken,
      jwt.verifyUser,
      jwt.verifyCompany,
      services.isServiceEnabled(process.env.SERVICE_BUREAU_SCORECARD_REPORT_ID),
    ],
    async (req, res) => {
      try {
        const result = await BureauScorecards.findByGenRequestId(
          req.query.request_id,
        );
        return res.status(200).send({
          data: result,
          message: 'Scorecard status fetched successfully!',
        });
      } catch (error) {
        return res.status(400).send({
          data: null,
