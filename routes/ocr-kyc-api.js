...
const localLogTemplate = {
  ...
  company_code: '',
  sub_company_code: '',
  vendor_name: process.env.OCR_SERVICE_PROVIDER,
  service_id: 0,
  api_name: '',
  request_id: '',
  raw_data: '',
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        let apiResponse="";
       if (process.env.OCR_SERVICE_PROVIDER === 'PUSHPAK') {
          apiResponse = await axios.request({
            
            url: process.env.PUSHPAK_URL,
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Basic ${process.env.PUSHPAK_AUTH_KEY}`,
            },
            data: {
        ...
        let vendorName = "KARZA";
        if(process.env.OCR_SERVICE_PROVIDER === "PUSHPAK"){
          vendorName = "PUSHPAK";
        }
        logErrorToS3(req, res, requestId, apiName, vendorName, error);
      }
    },
  );
};
