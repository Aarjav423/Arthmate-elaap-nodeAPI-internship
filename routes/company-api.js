...
module.exports = (app) => {
  ...
  app.get('/api/company/:page/:limit/:str', async (req, res) => {
    try {
      const { page, limit, str } = req.params;
      const companies = await Company.getPaginated(page, limit, str);
      const count = await Company.getPaginatedCount(page, limit, str);
  ...
  app.get('/api/loc-company', async (req, res) => {
    try {
      const products = await Product.getAllLocProduct();
      const companies = await Company.findCompanyIdArray(products);
      res.json(companies);
    } catch (error) {
      return res.status(400).send({
        error,
  ...
  app.post(
    ...
    [
      ...
      check('billing_name')
        .notEmpty()
        .withMessage('Vendor billing name is required'),
      ...
      check('service_delivery_state')
        .notEmpty()
        .withMessage('Service delivery state is required'),
      check('gstin')
        .optional({
          checkFalsy: true,
        })
  ...
  //msme company list
  app.get('/api/msme-company', async (req, res) => {
    try {
      const products = await Product.getAllMsmeProduct();
      const companies = await Company.findCompanyIdArray(products);
      res.json(companies);
    } catch (error) {
      return res.status(400).send({
