...
module.exports = (app) => {
  ...
  app.get('/api/company_services/:_company_id', async (req, res) => {
    try {
      ...
      const findCompanyIdResp =
        await CompanyServices.findOneWithCompanyId(companyId);
      if (!findCompanyIdResp)
        throw {
          message: 'Error while fetching services for selected company',
        };

      // if record is found but services are empty send an empty array
      if (findCompanyIdResp.services === '')
        throw {
          message: 'No service found for this company',
        };

      // convert the comma separated values to an array and send
      var isMultipleservices =
        findCompanyIdResp.services.toString().indexOf(',') > -1;
      const serviceArr = isMultipleservices
        ? findCompanyIdResp.services.split(',')
        : [findCompanyIdResp.services];
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const findCompanyResp = await CompanyServices.findOneWithCompanyId(
          servicesData.company_id,
        );
        ...
        if (findCompanyResp) {
          if (findCompanyResp.services) {
            var isMultipleservices =
              findCompanyResp.services.toString().indexOf(',') > -1;
            const serviceArr = isMultipleservices
              ? findCompanyResp.services.split(',')
              : [findCompanyResp.services];
            ...
            const updateResp =
              await CompanyServices.updateServices(findCompanyResp);
            if (!updateResp)
              throw {
            ...
            const updateNewServiceId =
              await CompanyServices.updateServices(findCompanyResp);
            if (!updateNewServiceId)
              throw {
          ...
          const addCompanyServiceResp =
            await CompanyServices.addNew(servicesData);
          if (!addCompanyServiceResp)
            throw {
  ...
  app.post(
    '/api/company_services/get_service_pc',
    [jwt.verifyToken, jwt.verifyCompany],
    async (req, res) => {
      try {
        ...
        if (!serviceFindRes.length || serviceFindRes.length == '0')
          return res.status(400).json({
        ...
        serviceFindRes.forEach(async (service) => {
          const result = await s3helper.fetchJsonFromS3(
            service.file_s3_path.substring(
              service.file_s3_path.indexOf('services'),
            ),
          );
          if (!result)
            throw {
          ...
          const item = {
            ...
            request: {
              ...
              body: {
                mode: 'raw',
                raw:
                  service.url === 'api/get_va_num' ||
                  service.url === 'api/emandate_debit'
