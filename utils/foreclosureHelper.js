...
const uploadWebhookDataToS3 = async (dataParams) => {
  try {
    const { company_id, product_id, webhookData, eventKey, foreclosureId } =
      dataParams;
    ...
    const reqKey = `WEBHOOK-NOTIFY/${company_id}/${product_id}/${foreclosureId}/${filename}/${Date.now()}.txt`;
...
const uploadPartnerResponseToS3 = async (dataParams) => {
  try {
    const { company_id, product_id, responseData, foreclosureId, webhookData } =
      dataParams;
    //upload response data to s3 .
    filename = Math.floor(10000 + Math.random() * 99999) + '_client_res';
    const resKey = `WEBHOOK-NOTIFY/${company_id}/${product_id}/${foreclosureId}/${filename}/${Date.now()}.txt`;
    ...
    if (uploadWebhookResponse) {
      ...
      webhookData.transaction_id = foreclosureId;
